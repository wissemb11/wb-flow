...
function autopilotRisks(waves, text) {
  ...
  for (const w of waves) {
    ...
    // 2c. A wave whose cells are ALL held is blocked, not closed — and the
    // distinction is load-bearing. A held cell is dropped from `w.routed`, so
    // without this rule such a wave contributes no risk at all; `risks` comes
    // back empty and the "Why not `--wave=all`" section flips from a refusal
    // into an endorsement while the thing that wave gates is still ungated.
    //
    // Measured 2026-08-22 on plan_broadcast_20260821.md: annotating wave E's
    // gate cell with `HELD` rendered the wave `✅ closed` and printed
    // "`--wave=all` is defensible here" — on a plan with an unsigned
    // publication gate and six irreversible public-action rows still open.
    // The safest possible annotation produced the most dangerous run-book.
    if (!w.cells && w.heldCells) {
      risks.push({
        wave: w.label,
        kind: 'gate',
        text: '**Wave ' + w.label + ' is held, not closed** — ' + w.heldCells + ' cell'
            + (w.heldCells === 1 ? ' carries' : 's carry') + ' an explicit hold marker, so nothing in it '
            + 'can dispatch. Whatever it gates is still ungated.',
      });
    }
  }
...
function render(planRel, waves, risks, opts) {
  ...
  // …but "no live cells" is not the same as "closed". A plan whose every wave is
  // HELD also has zero live cells, and calling that closed is the most dangerous
  // form of this bug: it declares finished a plan whose gates were never signed.
  // Distinguish the two by whether anything is holding. (2026-08-22)
  const heldTotal = waves.reduce(function (a, w) { return a + (w.heldCells || 0); }, 0);
  if (!live.length && heldTotal) {
    L.push('> Generated by `wb-flow next` — **derived from this plan**, not authored.');
    L.push('');
    L.push('⏸️ **Nothing can dispatch — every remaining cell is HELD.** This plan is **not** closed: '
         + heldTotal + (heldTotal === 1 ? ' cell carries' : ' cells carry') + ' an explicit hold marker. '
         + 'Whatever they gate is still ungated. Resolve the hold, then regenerate this block.');
    L.push('');
    for (const w of waves) {
      if (!w.heldCells || !w.heldParsed) continue;
      for (const h of w.heldParsed) {
        L.push('- **Wave ' + w.label + '** — `' + h.command + '` (' + h.reason + ')');
      }
    }
    L.push('');
    return L.join('\n');
  ...
  for (const w of waves) {
    if (!w.cells) {
      // A wave with no runnable cells is `✅ closed` ONLY if nothing in it is
      // held. An all-held wave still has open work — rendering it as closed is
      // how an unsigned gate comes to look finished. See rule 2c above.
      if (w.heldCells) {
        L.push('| **' + w.label + '** | 0 | — | ' + fmtMinutes(w.minutes) + ' | ⏸️ **held** ('
             + w.heldCells + ' cell' + (w.heldCells === 1 ? '' : 's') + ') |');
      } else {
        L.push('| ' + w.label + ' | 0 | — | — | ✅ closed |');
      }
      continue;
    }
