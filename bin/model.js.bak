...
function detect(opts) {
  const o = opts || {};
  const found = { opencode: null, agy: null, codex: null, claude: null, slugs: [], providers: [] };
...
function loadCustomModels(opts) {
  let filepath = (opts && opts.modelsFile) || process.env.WB_MODELS_FILE;
  if (!filepath) {
    const localFile = path.join(process.cwd(), '.wb', 'models.json');
    if (fs.existsSync(localFile)) filepath = localFile;
    else {
      try {
        const home = os.homedir();
        const userDotWb = path.join(home, '.wb', 'models.json');
        const userConfig = path.join(home, '.config', 'wb-flow', 'models.json');
        if (fs.existsSync(userDotWb)) filepath = userDotWb;
        else if (fs.existsSync(userConfig)) filepath = userConfig;
      } catch {}
    }
  }
...
module.exports = {
  run, detect, proposeRoster, pickForRole, renderRosterTable, readRoster,
