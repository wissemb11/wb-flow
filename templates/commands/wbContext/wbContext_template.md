...
/wbContext <monorepo-root>/ --scope=global           # monorepo-wide survey
```
...
| Monthly sanity check across the monorepo | `/wbContext <monorepo-root>/ --scope=global` |
...
> For deeper reading: [`wbContext_practical.md`](https://flow.wbc-ui.com/commands/wbContext/wbContext_practical) (or the `_eli5_`, `_expert_`, `_examples_` siblings).
...
| `--archive` | `-A` | **Universal** (`_shared/output_conventions.md` §13). Consolidate first, then retire every superseded `<DD>/contexts/` folder into `.wb/workflows/archives/` at the same depth. `--archive=all` sweeps every category; `--dry-run` previews. Shell out to `wb-flow archive` — never `mv` by hand. Never implied by another flag. |
...
/wbContext <previous_output_file>   # consolidate mode — absorb every still-open item from older contexts/ files, then repair in place
/wbContext <previous_output_file> --archive   # …and then retire the contexts/ folders it just superseded
```
