...
For GitHub Pages specifically, ensure the deploy config in `.wb/workflows/deploy.md` sets `base-url` to the Pages URL.
...
> For deeper reading: [`wbDeploy_practical.md`](https://flow.wbc-ui.com/commands/wbDeploy/wbDeploy_practical) (or the `_eli5_`, `_expert_`, `_examples_` siblings).
...
| `--archive` | `-A` | **Universal** (`_shared/output_conventions.md` §13). Consolidate first, then retire every superseded `<DD>/deployments/` folder into `.wb/workflows/archives/` at the same depth. `--archive=all` sweeps every category; `--dry-run` previews. Shell out to `wb-flow archive` — never `mv` by hand. Never implied by another flag. |
...
/wbDeploy <previous_output_file>   # consolidate mode — absorb every still-open item from older deployments/ files, then repair in place
/wbDeploy <previous_output_file> --archive   # …and then retire the deployments/ folders it just superseded
```
