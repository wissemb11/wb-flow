...
- **[`plan_state_management`](plan_state_management.md)** — the five task states (`⬜` `✅` `⏸️` `🚫` `🔄`) and the `--open`/`--def`/`--can` override flags shared by `/wbWork`, `/wbValid`, `/wbPlan`. Read when you need to defer, cancel, or re-open tasks without re-executing them.
...
- **[`command_glossary`](command_glossary.md)** — **every `/wb*` command and every `wb-flow` subcommand/flag in one table**, with bash, Python and Node equivalents. Explains the split that runs through the whole system: 28 of the 33 commands are *agent-only prompts*, 5 are *deterministic CLI with tests*.
- **[`cli-subcommands`](cli-subcommands.md)** — `wb-flow model`, `snap`, `wave` and `watch` in depth, plus the universal `--snap` / `--snap-copy` flag.
...
| Look up any command or flag, or call one from Python/Node | [`command_glossary`](command_glossary.md) |
